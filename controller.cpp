...
string ReaderController::browseBook(Book &book)
{
	...
	switch(err)
	{
		...
		case noError:
			{
				Content *content;
				ErrorCode errorCode=server->previewBookContent(user,book);
				//cout<<"wang!"<<endl;
				switch(errorCode)
				{
					...
					case noError:
					{
						...
						bool found=0;
						content=new TxtContent(book["Url"],found);
						if(found==1)
						{
							return content->show();
							}
...
string ReaderController::listBorrowingBooks(const User &_user)
{
	...
	CompleteMatchingSearch key=CompleteMatchingSearch(multiset<Field>{Field("Username",(_user["Username"])),Field("Type","Borrowing"),Field("Status","Accepted")});
	//cout<<"that"<<endl;
